...
<script>
$(function(){
  $('#tasksTable').DataTable({ responsive:true, language:{ url: "/{{TENANT_DIR}}/public/js/fr-FR.json" } });
  $('#btnNewTask').on('click', function(){
    // simple prompt demo; remplacer par modal
    var titre = prompt('Titre tâche');
    if(!titre) return;
    $.post('/{{TENANT_DIR}}/pages/crm/actions.php', { action:'add_task', titre: titre }, function(r){
  ...
  $('.btn-delete-task').on('click', function(){
    ...
    var id = $(this).data('id');
    $.post('/{{TENANT_DIR}}/pages/crm/actions.php', { action:'delete_task', id: id }, function(r){
      if(r.success) location.reload(); else alert(r.error || 'Erreur');
    }, 'json');
  });
  ...
  $('.btn-complete').on('click', function(){
    var id = $(this).data('id');
    $.post('/{{TENANT_DIR}}/pages/crm/actions.php', { action:'complete_task', id: id }, function(r){
      if(r.success) location.reload(); else alert(r.error || 'Erreur');
    }, 'json');
  });
});
