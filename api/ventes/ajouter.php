    ...
    if ($client_id && !empty($produits)) {
        ...
        foreach ($produits as $prod) {
            ...
            $conn->prepare("
                ...
                (produit_id, depot_source_id, quantite, type, reference_table, reference_id, utilisteur_id date_mouvement) 
                VALUES (?, ?, ?, 'vente', 'ventes', ?, ?, NOW())
            ")->execute([$produit_id, $depot_id, $quantite, $vente_id, $utilisateur_id]);
            }
        ...
        if ($montant_verse_devise > 0 ) {
            ...
            AND mp.id = ?
        ");
        $stmt->execute([$caisse_id, $mode_paiement_id]);
        $caisseType = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$caisseType) {
            throw new Exception("Type de caisse non configuré pour ce mode de paiement");
        }
...
header("Location: /eaglesuite/index.php?page=ventes");
exit;
