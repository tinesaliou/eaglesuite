    ...
    <div class="card mb-3 shadow-sm">
        <div class="card-body">
            <?php if (!empty($retour['client_nom'])): ?>
                <p><strong>Client :</strong> <?= htmlspecialchars($retour['client_nom'], ENT_QUOTES, 'UTF-8') ?></p>
            <?php elseif (!empty($retour['fournisseur_nom'])): ?>
                <p><strong>Fournisseur :</strong> <?= htmlspecialchars($retour['fournisseur_nom'], ENT_QUOTES, 'UTF-8') ?></p>
            <?php endif; ?>

            <p><strong>Dépôt :</strong> <?= htmlspecialchars($retour['depot_nom'], ENT_QUOTES, 'UTF-8') ?></p>
            <p><strong>Date retour :</strong> <?= date("d/m/Y H:i", strtotime($retour['date_retour'])) ?></p>
            <p><strong>Utilisateur :</strong> <?= htmlspecialchars($retour['utilisateur_nom'], ENT_QUOTES, 'UTF-8' ?? '-') ?></p>
            <p><strong>Raison :</strong> <?= nl2br(htmlspecialchars($retour['raison'], ENT_QUOTES, 'UTF-8')) ?></p>
        </div>
    </div>
               ...
               <thead class="table-dark">
                ...
                <tbody>
                    <?php foreach ($details as $d): ?>
                        <tr>
                            <td><?= htmlspecialchars($d['produit_nom'], ENT_QUOTES, 'UTF-8') ?></td>
