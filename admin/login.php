...
require_once __DIR__ . '/../tenants/load.php';

// Déjà connecté → dashboard admin
if (is_admin_logged()) {
    header('Location: /eaglesuite/admin/index.php?page=dashboard');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($login === '' || $pass === '') {
        ...
        // --------------------------------------------------------
        // 1) On cherche dans la table admin_users
        //    → par username OU par email
        // --------------------------------------------------------
        $sql = "
        ...
        // --------------------------------------------------------
        // 2) Fallback dans utilisateurs (admin/superadmin)
        // --------------------------------------------------------
        if (!$admin) {
            $sql = "
                SELECT u.* , r.nom FROM utilisateurs u
                JOIN roles r ON u.role_id = r.id
                WHERE (username = :login OR email = :login)
                AND r.nom IN ('admin','superadmin')
                LIMIT 1
            ";
        ...
        if ($admin && password_verify($pass, $admin['mot_de_passe'])) {
            ...
            $_SESSION['admin_id'] = $admin['id'] ?? $admin['idAdmin'] ?? null;
            $_SESSION['admin_name'] = $admin['nom'] ?? $admin['username'] ?? $admin['email'] ?? 'Admin';
  ...
  <main class="auth-wrap" aria-labelledby="login-title">
    ...
    <section class="card-auth" aria-label="Formulaire de connexion">
      <div class="brand">
        <div class="logo logo-pulse" aria-hidden="true">
          <img src="/eaglesuite/public/icone/eaglesuite.png" alt="EagleSuite Logo">
        </div>
        <div>
      ...
      <h3 id="login-title" class="title">Espace d'administration</h3>
      <p class="lead"> Entrez vos identifiants sécurisés.</p>
      ...
      <form method="POST" novalidate>
        <label for="login" class="form-label">Nom utilisateur</label>
        <input id="login" name="login" type="test" autocomplete="username" class="form-control mb-3" placeholder="Utilisateur" required>
    ...
    <aside class="visual" aria-hidden="true">
      ...
      <div class="big-logo" role="img" aria-label="Logo EagleSuite">
        <img src="/eaglesuite/public/icone/eaglesuite_logo.png" alt="">
      </div>
    </aside>
