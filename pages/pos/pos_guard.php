...
if (isset($_SESSION['pos_last_activity']) &&
    ...
    session_unset();
    session_destroy();
    header("Location: /eaglesuite/index.php?page=pos-select-caisse&timeout=1");
    exit;
}
...
if (!$stmt->fetchColumn()) {
    session_destroy();
    header("Location: /eaglesuite/index.php?page=pos-select-caisse&inactive=1");
    exit;
}
...
if (!$stmt->fetchColumn()) {
    session_destroy();
    header("Location: /eaglesuite/index.php?page=pos-select-caisse&unauthorized=1");
    exit;
}
...
if (!$session_caisse_id) {
    header("Location: /eaglesuite/index.php?page=pos-open");
    exit;
}
