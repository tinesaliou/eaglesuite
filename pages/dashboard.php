...
$stmt = $conn->query("SELECT ct.id, tc.libelle AS nom, ct.solde_actuel FROM caisse_types ct INNER JOIN types_caisse tc ON ct.type_caisse_id = tc.id");
$caisses = $stmt->fetchAll(PDO::FETCH_ASSOC);

$stmt = $conn->query("
    SELECT oc.date_operation, oc.type_operation, oc.montant, oc.description, tc.libelle AS caisse
    FROM operations_caisse oc
    JOIN caisse_types ct ON ct.id = oc.caisse_type_id
        ...
        <div class="card">
          ...
          <div class="card-body">
            <?php foreach($caisses as $c): ?>
              <div><strong><?= htmlspecialchars($c['nom'], ENT_QUOTES, 'UTF-8') ?></strong> — Solde: <?= number_format($c['solde_actuel'],0,',',' ') ?> FCFA</div>
            <?php endforeach; ?>
            <hr>
            <div class="table-responsive">
